...
const OrderParts = () => {
  ...
  const { data: part, isLoading } = useQuery({
    queryKey: ["part"],
    queryFn: () =>
      fetch(`  https://rockauto-server.vercel.app/parts/${_id} `).then((res) => res.json()),
  });
  ...
  const sendOrder = (e) => {
    ...
    // const newQuantity = part.quantity - orderedQuantity;
    ...
    fetch(" https://rockauto-server.vercel.app/order", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row  hero items-center justify-around  lg:min-h-screen">
      <div className=" max-w-sm lg:max-w-md md:max-w-xl p-6 shadow-2xl rounded-lg ">
        <form onSubmit={sendOrder}>
          ...
          <input
            ...
            disabled={isDisabled}
            value="ORDER"
            className="btn w-full btn-primary my-4"
      ...
      <div className="lg:max-w-sm max-w-xs md:max-w-md">
        ...
        <h2 className="lg:text-5xl text-2xl md:text-4xl font-bold text-secondary mb-6">
          {part.name}
        </h2>
        <h2 className="lg:text-4xl text-2xl md:text-4xl mb-6 lg:mb-12">
          {" "}
          $<span className="font-bold text-black ">{part.price}</span>{" "}
        </h2>
        <h3 className="text-xl ">
          {" "}
          <span className="font-semibold"> Available Quality :</span>{" "}
          {part.quantity}{" "}
        </h3>
